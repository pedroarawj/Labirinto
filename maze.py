...
class Maze:
    ...
    def mov_player(self, pos: (int, int)) -> None:
        # Deixa rastro (marca a posição anterior como caminho)
        player_pos = np.argwhere(self.M == Maze.PLAYER)
        if player_pos.size > 0:
            old_x, old_y = player_pos[0]
            self.M[old_x, old_y] = Maze.PATH  # marca rastro
        
        # Move o jogador se for corredor ou prêmio
        if self.M[pos[0], pos[1]] in [Maze.HALL, Maze.PRIZE]:
